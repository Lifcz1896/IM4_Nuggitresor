...
(function () {
  ...
  function mobileLink(href, name, label, svg) {
    return `<a href="${href}" class="mobile-nav-link${active(name) ? ' mobile-nav-link--active' : ''}">${svg}<span>${label}</span></a>`;
  }
  ...
  const iconHandbuch = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg>`;
  ...
  el.innerHTML = `
    ...
    <nav class="sidebar-nav">
      ...
      ${link('handbuch.html',  'handbuch',  'Handbuch',   iconHandbuch)}
    </nav>
  ...
  nav.innerHTML = `
    ${mobileLink('dashboard.html', 'dashboard', 'Dashboard', iconDashboard)}
    ${mobileLink('stats.html',     'stats',     'Statistik', iconStats)}
    ${mobileLink('profile.html',   'profile',   'Profil',    iconProfile)}
    ${mobileLink('handbuch.html',  'handbuch',  'Handbuch',  iconHandbuch)}
  `;
